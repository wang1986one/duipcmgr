...
#define PEB_BASE        (0x7FFDF000)
	...
	typedef struct _stProcessEntry 
	{
		DWORD	ProcessID;
		DWORD	ParentProcessID;
		CString strExeName;
	}PROCESSENTRY, *PPROCESSENTRY;
	typedef std::vector<PROCESSENTRY> ProcessEntryList;

	class CProcessMgr
	{
	public:
		static void UpdateProcessList(ProcessEntryList& ProcessEntry);

		static VOID	GetProcessNameByPID(DWORD dwProcessID,CString& strName);
		static DWORD GetProcessIDByName(LPCTSTR lpszProcessName);
		static HANDLE GetProcessHandleByPID(DWORD dwProcessID);
		static VOID GetProcessFullPath(DWORD dwProcessID,CString& strFullPath);
		static VOID KillProcess(DWORD dwProcessId);

		static BOOL ExecuteExe(LPCTSTR lpExecutePath, LPCTSTR lpParam, DWORD* pdwExitCode,BOOL bRequiredAdmin =FALSE , BOOL bWaitExitCode = FALSE);
	};
}
